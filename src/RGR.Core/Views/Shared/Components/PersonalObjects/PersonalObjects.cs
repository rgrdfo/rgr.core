...
namespace RGR.Core.Views.Shared.Components
{
    public class PersonalObjects : ViewComponent
    {
        ...
        {
            var grouped = Source.GroupBy(l => l["Type"]).Select(l => l.ToList());            
            return View(await Task.Run(() => Render(grouped)));
        }


        // Вкладка Квартира (id=4)
        public static HtmlString StringBuilderFlat(List<ShortPassport> flats)
        {
            StringBuilder sb = new StringBuilder("<div id = \"typeObj4\" class=\"tab - pane \">");
            if (flats != null)
            {
                sb.Append($"< h5 >< b > Новых совпадений по объектам:{(flats != null ? flats.Count() : 0)} </ b ></ h5 >");
                foreach (var flat in flats)
                {
                    sb.Append($"<div class=\"search-box-head\">");
                    ...
                    sb.Append($"< div class=\"row\"><div class=\"col-lg-3\" style=\"border-bottom:none;\"><h1><span>{flat["Address"]}<br/>{flat["City"]}</span></h1>");
                    sb.Append($"< img src =\"{flat["Logo"]}\"></div>");
                    sb.Append($"<div class=\"col-lg-1\"><h1>{flat["DateToShow"]}</h1><br />ID: {flat["Id"]:0000000}</div>");
                    ...
                    sb.Append($"<div class=\"col-lg-1\"><h1>{flat["Area"]} м²</h1><br />кухня {flat["KitchenArea"]}<br />жилая {flat["LivingArea"]}</div>");
        ...
        // Вкладка Дом (id=1)
        public static HtmlString StringBuilderHouse(List<ShortPassport> houses)
        {
            StringBuilder sb = new StringBuilder("<div id = \"typeObj1\" class=\"tab - pane in active\">");
            if (houses != null)
            {
                sb.Append($"<div class=\"search-box-head\">");
                ...
                foreach (var house in houses)
                {
                    ...
                    sb.Append($"< div class=\"row\"><div class=\"col-lg-3\" style=\"border-bottom:none;\"><h1><span>{house["Address"]}<br/>{house["City"]}</span></h1>");
        ...
        // Вкладка Гараж (id=3)
        public static HtmlString StringBuilderGarage(List<ShortPassport> garages)
        {
            StringBuilder sb = new StringBuilder("<div id = \"typeObj3\" class=\"tab - pane\">");
            if (garages != null)
            {
                sb.Append($"< div class=\"tab - content\"><div id = \"typeObj3\" class=\"tab - pane\">");
        ...
        public static HtmlString StringBuilderLand(List<ShortPassport> lands)
        {
            StringBuilder sb = new StringBuilder("<div id = \"typeObj2\" class=\"tab - pane\">");
            if (lands != null)
            {
                sb.Append($"< div class=\"tab - content\"><div id = \"typeObj4\" class=\"tab - pane\">");
        ...
        public static HtmlString StringBuilderOffice(List<ShortPassport> offices)
        {
            StringBuilder sb = new StringBuilder("<div id = \"typeObj5\" class=\"tab - pane\">");
            if (offices != null)
            {
                sb.Append($"< div class=\"tab - content\"><div id = \"typeObj4\" class=\"tab - pane\">");
        ...
        public static HtmlString Render(IEnumerable<List<ShortPassport>> grouped)
        {
            ...
            else
            {
                ...
                var flats = grouped.FirstOrDefault(g => g.Any(f => f["Type"].Equals(EstateTypes.Flat)));
                var houses = grouped.FirstOrDefault(g => g.Any(f => f["Type"].Equals(EstateTypes.House)));
                var lands = grouped.FirstOrDefault(g => g.Any(f => f["Type"].Equals(EstateTypes.Land)));
                var garages = grouped.FirstOrDefault(g => g.Any(f => f["Type"].Equals(EstateTypes.Garage)));
                var offices = grouped.FirstOrDefault(g => g.Any(f => f["Type"].Equals(EstateTypes.Office)));
                StringBuilderHouse(houses);
                StringBuilderLand(lands);
                StringBuilderGarage(garages);
                StringBuilderFlat(flats);
                StringBuilderOffice(offices);
                sb.Append($"</div></div>");               
                return new HtmlString(sb.ToString());
            }
